...
(function () {
  ...
  const rowPreviewImg = $("#rowPreviewImg");
    ...
    $$(".project-row", projectList).forEach((row) => {
      ...
      if (!isTouchDevice) {
        row.addEventListener("mouseenter", () => {
          rowPreviewImg.src = row.dataset.cover;
          rowPreview.classList.add("is-visible");
        });
        row.addEventListener("mousemove", (e) => {
          rowPreview.style.left = `${e.clientX}px`;
          rowPreview.style.top = `${e.clientY}px`;
        });
        row.addEventListener("mouseleave", () => rowPreview.classList.remove("is-visible"));
      }
    });
  }
  ...
  emailCopy.addEventListener("click", async () => {
    const email = "hello@noalaurent.com";
    try {
